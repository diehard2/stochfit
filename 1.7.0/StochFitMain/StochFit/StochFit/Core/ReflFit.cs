...
namespace StochasticModeling
{
    public class ReflFit:BoxReflFitBase
    {
        ...
        public override string StochFit(double[] parampercs, int iterations)
        {
            ...
            double[] info = new double[9*1000];

            Calculations.ConstrainedStochFit(InfoStruct, parameters, CovarArray, parameters.Length, info, ParamArray, ChiSquareArray,ref Size);
            ...
            {
                if (ModelChooser(ParamArray, ChiSquareArray, CovarArray, info, Size, parameters.Length, InfoStruct))
                {
