...
CReflCalc::CReflCalc():exi(NULL),eyi(NULL),xi(NULL),yi(NULL), m_bReflInitialized(FALSE), sinthetai(NULL), m_ckk(NULL)
{}
...
void CReflCalc::InitializeScratchArrays(int EDPoints)
{
	//Create the scratch arrays for the reflectivity calculation
 	m_ckk = (MyComplex *)_mm_malloc(sizeof(MyComplex )*EDPoints*m_iuseableprocessors,64);
	m_dkk = (double*)_mm_malloc(sizeof(double)*EDPoints*m_iuseableprocessors,64);
	m_cak = (MyComplex *)_mm_malloc(sizeof(MyComplex )*EDPoints*m_iuseableprocessors,64);
	m_crj = (MyComplex *)_mm_malloc(sizeof(MyComplex )*EDPoints*m_iuseableprocessors,64);
	m_drj = (double*)_mm_malloc(sizeof(double)*EDPoints*m_iuseableprocessors,64);
	m_cRj = (MyComplex *)_mm_malloc(sizeof(MyComplex )*EDPoints*m_iuseableprocessors,64);
...
void CReflCalc::CalculateReflectivityNoOpt(CEDP *EDP)
{
int counter = m_idatapoints;
	...
	if(m_dQSpread < 0.005 || exi == NULL)
	{
		MyRFNoOpt(sinthetai, sinsquaredthetai, m_idatapoints, reflpt, EDP);

	}
	else
	{
		MyRFNoOpt(qspreadsinthetai, qspreadsinsquaredthetai, 13*m_idatapoints, qspreadreflpt, EDP);
		QsmearRf(qspreadreflpt, reflpt, m_idatapoints);
	}
