	...
	#region ContextMenu
		...
		public void CopyEmf(bool isShowMessage)
		{
			if (_masterPane != null)
			{
				// Threaded copy mode to avoid crash with MTA
				// Contributed by Dave Moor
                Thread ct = new Thread(delegate() { ClipboardCopyThreadEmf(null); });
				//ct.ApartmentState = ApartmentState.STA;
				ct.SetApartmentState(ApartmentState.STA);
				ct.Start();
				ct.Join();
        ...
        public void CopyEmf(bool isShowMessage, PointF mousepos)
        {
            if (_masterPane != null)
            {
                // Threaded copy mode to avoid crash with MTA
                // Contributed by Dave Moor
                Thread ct = new Thread(delegate() { ClipboardCopyThreadEmf(mousepos); });
                //ct.ApartmentState = ApartmentState.STA;
                ct.SetApartmentState(ApartmentState.STA);
                ct.Start();
                ct.Join();
			...
			if ( _masterPane != null )
			{
				...
				if ( _saveFileDialog.ShowDialog() == DialogResult.OK )
				{
					...
					if ( myStream != null )
					{
						...
						{
							myStream.Close();
							SaveEmfFile( _saveFileDialog.FileName,null );
						}
						else
						{
		...
		/// <see cref="SaveAs()" />, which allows for Bitmap or EMF formats.
		/// </remarks>
		public void SaveAsEmf(PointF? mousepos)
		{
			if ( _masterPane != null )
			{
				...
				if ( _saveFileDialog.ShowDialog() == DialogResult.OK )
				{
					...
					if ( myStream != null )
					{
						...
						SaveEmfFile(_saveFileDialog.FileName, mousepos);
					}
				}
			}
		}
