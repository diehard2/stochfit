...
class ParrattReflectivity
{
  ...
  private:
    ...
    void ReflectivityCalcCoreImpl(const LayerStack& ls, std::span<const double> sinthetai, std::span<const double> sin2thetai,
                                  std::span<double> out, int q_end = std::numeric_limits<int>::max());

    void ReflectivityCalc(const LayerStack& ls, int q_end = std::numeric_limits<int>::max());
