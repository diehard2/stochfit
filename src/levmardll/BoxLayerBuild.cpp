...
void BuildBoxLayers(const BoxReflSettings& rs, std::span<const double> p, bool one_sigma, BoxLayers& out)
{
    const int nl = rs.Boxes + 2;
    const double rhofactor = 1e-6 * rs.Wavelength * rs.Wavelength / (2.0 * std::numbers::pi);
