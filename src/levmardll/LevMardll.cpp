...
static void LevmarReflResidual(double* p, double* x, int m, int n, void* data)
{
    ...
    BuildBoxLayers(*self->rs, std::span<const double>(p, m), self->rs->OneSigma, self->layers);
...
extern "C" EXPORT int FastReflfit(const uint8_t* inBuf, int /*inLen*/, uint8_t* outBuf, int maxLen)
{
    ...
    LevmarReflTask task{ParrattReflectivity(ToReflSettings(rs)),
...
extern "C" EXPORT int RhoGenerate(const uint8_t* inBuf, int /*inLen*/, uint8_t* outBuf, int maxLen)
{
    ...
    auto result = StochFitProto::CreateRhoGenerateResult(fbb, fbb.CreateVector(ed), fbb.CreateVector(boxED));
    return finish_into(fbb, result, outBuf, maxLen);
}
...
struct BoxSolution
{
    ...
    bool operator<(const BoxSolution& o) const
    {
        return score < o.score;
    }
};

static bool IsReasonable(const BoxSolution& sol, const BoxReflSettings& rs, double cutoff)
{
    BoxLayers layers;
...
extern "C" EXPORT int StochFitBoxModel(const uint8_t* inBuf, int /*inLen*/, uint8_t* outBuf, int maxLen)
{
    ...
    double bestchisquare = 0;
    {
        LevmarReflTask tmp{.parratt = ParrattReflectivity(ToReflSettings(rs)),
                           .objective = ReflectivityObjective(ReflectivityObjective::Type{rs.FitFunc}),
                           .layers = {},
                           .Realrefl = rs.Refl,
...
#pragma omp parallel
    {
        ...
        auto IRandom = [&](double max, double min) { return std::uniform_real_distribution<double>(min, max)(randgen); };
...
#pragma omp for schedule(runtime)
        for (int i = 0; i < rs.Iterations; i++) {
            ...
            if (rs.UL.empty()) {
                dlevmar_dif(LevmarReflResidual, locparameters.data(), xvec.data(), paramsize, QSize, 500, opts, locinfo.data(), work.data(),
                            covar.data(), (void*) (&task));
            } else {
                dlevmar_bc_dif(LevmarReflResidual, locparameters.data(), xvec.data(), paramsize, QSize, rs.LL.data(), rs.UL.data(), nullptr,
                               500, opts, locinfo.data(), work.data(), covar.data(), (void*) (&task));
            }
            ...
            if (locinfo[1] < bestchisquare && IsReasonable(localanswer, rs, rs.ParamPercs[6])) {
                bool unique = true;
...
#pragma omp critical(AddVecs)
    ...
    auto result =
        StochFitProto::CreateBoxStochFitResult(fbb, fbb.CreateVector(outParams), fbb.CreateVector(covarArray), fbb.CreateVector(infoOut),
                                               fbb.CreateVector(paramArray), fbb.CreateVector(chiSquareArray), n);
    return finish_into(fbb, result, outBuf, maxLen);
}
