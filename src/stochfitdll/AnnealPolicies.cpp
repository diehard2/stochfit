...
StunPolicy::StunPolicy(double initTemp, double slope, int platIter, double gamma, int stunFunc, int tempIter, bool adaptive)
...
double StunPolicy::fSTUN(double val, double bestE) const
{
    ...
    switch (m_stunFunc) {
        case 0:
            return -std::exp(-x);
        case 1:
            return std::sinh(x) - 1.0;
        default:
            return std::log(x + std::sqrt(x * x + 1.0)) - 1.0;  // asinh - 1
    }
}
