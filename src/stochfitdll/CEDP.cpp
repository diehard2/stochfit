...
std::pair<int, int> CEDP::GetOffSets() const
{
    ...
    const int sup_offset =
        static_cast<int>(std::ranges::find_if(m_DEDP, [&](const auto& v) { return v.real() != front; }) - m_DEDP.begin());

    const int sub_offset =
        static_cast<int>(m_DEDP.size()) - 1 -
        static_cast<int>(std::ranges::find_if(m_DEDP | std::views::reverse, [&](const auto& v) { return v.real() != back; }) -
                         m_DEDP.rbegin());
...
double CEDP::Get_WaveConstant() const
{
