...
ParameterStepper::ParameterStepper(Config cfg) : m_cfg(cfg), m_rng(std::random_device{}())
{
}
...
void ParameterStepper::Step(ParamVector& params)
{
    ...
    if (perc > normTop) {
        ...
        params.SetImpNorm(std::uniform_real_distribution<double>(params.GetImpNorm() * (1.0 - m_cfg.stepSize),
