...
void ReflectivityObjective::FillResiduals(std::span<const double> model, std::span<const double> yi, std::span<const double> eyi,
                                          std::span<double> residuals, int low_q_offset, int high_q_offset) const
{
...
double ReflectivityObjective::Evaluate(std::span<const double> model, std::span<const double> yi, std::span<const double> eyi) const
{
    const int n = static_cast<int>(model.size());
    double score = 0.0;
...
double ComputeChiSquare(std::span<const double> model, std::span<const double> yi, std::span<const double> eyi)
{
    const int n = static_cast<int>(model.size());
    double chi = 0.0;
    for (int i = 0; i < n; ++i) {
