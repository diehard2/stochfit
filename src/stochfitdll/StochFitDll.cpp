...
extern "C" EXPORT int GetData(uint8_t* outBuf, int maxLen)
{
    ...
    auto result = StochFitProto::CreateGetDataResult(fbb, zVec, rhoVec, qVec, rfVec, snap.roughness, snap.chiSquare, snap.goodnessOfFit,
...
extern "C" EXPORT int GetRunState(uint8_t* outBuf, int maxLen)
{
    ...
    auto result =
        StochFitProto::CreateGetRunStateResult(fbb, s.roughness, s.filmAbsInput, s.surfAbs, s.temperature, s.impNorm, s.avgfSTUN,
                                               s.bestSolution, s.chiSquare, s.goodnessOfFit, edVec, 0 /*iteration filled by caller*/);
