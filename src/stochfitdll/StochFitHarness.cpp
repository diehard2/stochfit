...
StochFit::StochFit(const ReflSettings& InitStruct, const StochRunState* state)
    ...
    : m_initStruct(InitStruct),
      ...
      m_stepper({InitStruct.Sigmasearch,
                 InitStruct.AbsorptionSearchPerc,
                 InitStruct.NormalizationSearchPerc,
                 InitStruct.Paramtemp})
{
    m_bupdated       = false;
    ...
    Zinc.resize(m_cEDP.Get_EDPPointCount());
    Qinc.resize(m_cRefl.GetDataCount());
    Rho.resize(m_cEDP.Get_EDPPointCount());
    ...
    deps.forceNorm = InitStruct.Forcenorm;
    ...
    case SaAlgorithm::Greedy:
        m_annealer.emplace(std::in_place_type<Anneal<GreedyPolicy>>,
            m_cEDP, m_parratt, m_objective, m_stepper, params, deps);
        break;
    case SaAlgorithm::Simulated:
        m_annealer.emplace(std::in_place_type<Anneal<SimulatedPolicy>>,
            m_cEDP, m_parratt, m_objective, m_stepper, params, deps,
            InitStruct.Inittemp, InitStruct.Slope, InitStruct.Platiter);
        break;
    case SaAlgorithm::Stun:
        m_annealer.emplace(std::in_place_type<Anneal<StunPolicy>>,
            m_cEDP, m_parratt, m_objective, m_stepper, params, deps,
            InitStruct.Inittemp, InitStruct.Slope, InitStruct.Platiter,
            InitStruct.Gamma, InitStruct.Gammadec, InitStruct.STUNfunc,
            InitStruct.STUNdeciter, InitStruct.Tempiter, InitStruct.Adaptive);
        break;
    }
...
int StochFit::Processing()
{
    try {
        ...
        if (!m_annealer) return -1;

        for (int isteps = 0;
             isteps < m_itotaliterations && !m_stop_requested.load();
             ++isteps) {

            const bool accepted =
                std::visit([&](auto& a){ return a.Iteration(params); },
                           *m_annealer);

            if (accepted || isteps == 0) {
                m_dChiSquare = std::visit(
                    [](const auto& a){ return a.GetLastChiSquare(); },
                    *m_annealer);
                m_dGoodnessOfFit = std::visit(
                    [](const auto& a){ return a.GetCurrentEnergy(); },
                    *m_annealer);
            }
            UpdateFits(isteps);
        }
        ...
        std::cerr << "[StochFit] Processing() caught exception: "
                  << ex.what() << std::endl;
...
int StochFit::ProcessingGPU()
{
    ...
    auto gpu_info  = detect_gpu();
    const int num_chains = (m_initStruct.GpuChains > 0)
                           ? m_initStruct.GpuChains
                           : gpu_info.max_chains;
    ...
    for (int done = 0;
         done < outer_total && !m_stop_requested.load();
         done += batch_size) {
        ...
        if (m_bupdated || (now - last_update) >= update_interval) {
            ...
            for (int i = 0; i < rps && i < static_cast<int>(GPU_MAX_BOXES + 2); i++) {
                if (i == 0)           params.SetSupphase(result.best_params[i]);
                else if (i == rps-1)  params.SetSubphase(result.best_params[i]);
                else                  params.SetMutatableParameter(i-1, result.best_params[i]);
            }
            params.setroughness(result.best_roughness);
            if (params.Get_UseSurfAbs())   params.setSurfAbs(result.best_surf_abs);
            if (params.Get_FixImpNorm())   params.setImpNorm(result.best_imp_norm);
            ...
            for (int i = 0; i < static_cast<int>(Rho.size()); i++) {
                ...
                Rho[i]  = m_cEDP.m_EDP[i].real() /
                           m_cEDP.m_EDP[m_cEDP.Get_EDPPointCount()-1].real();
            }
...
int StochFit::GetData(double* Z, double* RhoOut, double* Q, double* ReflOut,
...
{
    *RhoSize  = static_cast<int>(Rho.size());
    *ReflSize = static_cast<int>(Refl.size());
}
