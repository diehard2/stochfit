...
static bool LoadReflData(const std::string& path, std::vector<double>& q, std::vector<double>& refl, std::vector<double>& reflErr)
{
    std::ifstream f(path);
    if (!f) {
...
int main(int argc, char* argv[])
{
    ...
    harness.Stop();
    double totalSec = std::chrono::duration<double>(std::chrono::steady_clock::now() - wallStart).count();
